            ...
            @foreach ($categories as $cate)
                ...
                <div class="bg-[#4E1BE4] rounded-lg p-2 text-center">
                    <img class="rounded-lg h-40 w-full object-cover mx-auto mb-4"
                        src="{{ asset('storage/' . $cate->image_url) }}" alt="Category 1"
                        class="mx-auto mb-4">
                    <h2 class="text-[24px] font-semibold text-white mb-2">{{ $cate->name }}</h2>
                ...
                @foreach ($jobs as $job)
                    <div class="bg-white rounded-2xl p-6 mt-8 flex items-center border-gray-300 border-2 shadow-lg">
                        <div class="for-company-logo w-2/10">
                            <img class="rounded-lg mr-2 h-20 w-20 object-cover mx-auto mb-4"
                                src="{{ asset('storage/' . $job->user->company_logo) }}" alt="Company Logo">
                        </div>
                        <div>
                            <h2 class="text-[16px] font-semibold text-start text-[#4E4E4E] mb-2">
                                {{ $job->user->company_name ?? $job->user->name }}
                            </h2>
                            <h1 class="text-[20px] font-bold text-start text-[#4E4E4E]">
                                {{ $job->title }}
                            </h1>
                            <div class="flex gap-4 mt-2 mb-2">
                                <p class="text-[12px] bg-gray-500/25 p-2 rounded-lg font-medium text-[#696969]">
                                    {{ $job->employment_type }}
                                </p>
                                @php
                                    $range = explode('-', $job->salary_range); // ["350 ", " 500"]
                                @endphp
                                <p class="text-[12px] bg-gray-500/25 p-2 rounded-lg font-medium text-[#696969]">
                                    ${{ trim($range[0]) }} - ${{ trim($range[1]) }}
                                </p>
                                <p class="text-[12px] bg-gray-500/25 p-2 rounded-lg font-medium text-[#696969]">
                                    {{ $job->created_at->diffForHumans() }}
                                </p>
                            </div>
                            <p class="text-[#3B3B3B] text-[14px] font-medium">
